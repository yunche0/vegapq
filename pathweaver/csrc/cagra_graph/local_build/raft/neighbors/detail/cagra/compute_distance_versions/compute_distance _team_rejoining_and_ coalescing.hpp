  ...
  uint32_t* counter = nullptr)
{
  ...
  // TODO: Change this to dynamically allocate work load

  if(is_dynamic)
  {
        uint32_t team_size = 8;
        uint32_t lane_id = threadIdx.x % team_size;
        uint32_t team_id  = (threadIdx.x % 32) / team_size;
        uint32_t team_mask = 0xff << (team_id * team_size);
        while(true)
        {
              uint32_t i_global = 0;
              if(lane_id ==  0)
              {
                i_global = atomicAdd(counter, 4);
              }
              uint32_t i = __shfl_sync(team_mask, i_global, 0) + team_id;
              if(i >= knn_k * search_width) { break; }
                  ...
                  norm2 =
                    dataset_desc
                      .template compute_similarity<DATASET_BLOCK_DIM, TEAM_SIZE, raft::distance::L2Expanded>(
                        query_buffer, child_id, child_id != invalid_index, true, thres);
                  break;
                case raft::distance::InnerProduct:
                  norm2 = dataset_desc.template compute_similarity<DATASET_BLOCK_DIM,
                                                                  TEAM_SIZE,
              ...
              // Store the distance
              // TODO this needs to be changed too
              const unsigned lane_id = threadIdx.x % TEAM_SIZE;
              if (valid_i && lane_id == 0) {
                if (child_id != invalid_index) {
...
struct standard_dataset_descriptor_t
                                          ...
                                          ) const
  {
    ...
    {             
          // TODO: Sync with subwarp size to reduce and return
          // TODO: Until 50% of the vector use the format function

          const DISTANCE_T ignore_value = utils::get_max_value<DISTANCE_T>();

          // TODO Reduce local variable
          // TODO Use minimum data type
          const unsigned team_size = 8
          unsigned lane_id  = threadIdx.x % team_size;
          unsigned team_id  = ( threadIdx.x % 32 ) / team_size;
          const unsigned num_team = 32 / team_size;
          unsigned int team_mask = 0xff << (team_id * team_size);

          DATA_T dl_buff[team_size * num_team];
          
          // use valid to check jump
          while(true)
          {
          ...
          #pragma unroll
          for (uint8_t t = 0 ; t < num_team; t++)
          {
              uint32_t i = __shfl_sync(0xffffffff, dataset_i, t * team_size);
              const auto dataset_pt = ptr + i * ld;
              
              // Full 32 threads in warp coalescing loading
              __syncwarp();
              dl_buff[lane_id] = dataset_pt[lane_id + team_size * team_id];
          }

          // Compute distance
          // No need to consider bank conflict
          // TODO Unroll the loop and remove vector_mask
          DISTANCE_T norm2 = 0;                             // Optionality there can be 4 norm2, this can delete broadcasting cost
          uint32_t vector_mask = 0xff000000;
          #pragma unroll
          for (uint8_t v = 0; v < 32; v += team_size)
          {
                DISTANCE_T norm2_partial = (query_ptr[lane_id + v] - dl_buff[lane_id  + v]) * (query_ptr[lane_id + v] - dl_buff[lane_id + v]);
                for (uint32_t offset = 32 / 2; offset > 0; offset >>= 1) 
                {
                  norm2_partial += __shfl_xor_sync(0xffffffff, norm2_partial, offset);
                }
                norm2 += __shfl__sync(0xff000000, norm2_partial, 0);
                vector_mask = vector_mask >> team_size;
          }

          // Check if the distance is bigger than threshold
          if(norm2 > thres) 
          {
                norm2 = ignore_value;
          }          
          ...
          // Check vectors with work left
          uint32_t work_left_mask = ballot_sync(0xffffffff, norm2 != ignore_value);
          int team_id =( __popc(work_left_mask & ((1 << lane_id) - 1)) ) / team_size;
          ...
          // TODO: Using switch statement load half first
          #pragma unroll
          for (uint32_t e = 0; e < reg_nelem; e++)
          {
                norm2_1 = 0;
                if (valid) {
                          ...
                          {        
                                    ...
                                    for (uint32_t v = 0; v < vlen; v++) {
                                      ...
                                      const uint32_t kv = k + v;
                                      DISTANCE_T d = query_ptr[device::swizzling(kv)];
                                      norm2_1 += dist_op<DISTANCE_T, METRIC>(d, spatial::knn::detail::utils::mapping<float>{}(dl_buff[e].val.data[v]));
                                    }
                ...
                for (uint32_t offset = TEAM_SIZE / 2; offset > 0; offset >>= 1) 
                {
                  norm2_1 += __shfl_xor_sync(team_mask, norm2_1, offset);
                }

                // add break if norm2_1 + norm2_2 bigger than thres
                if(norm2 + norm2_1 > thres) 
                { 
                    // printf("Jumped\n");
                    return ignore_value; 
                }
