...
#define _CLK_REC(V)
  ...
  // TODO atomic add
  // auto counter = reinterpret_cast<std::uint32_t*>(smem_work_ptr + 1);
  ...
  while (1) {
    ...
    // TODO: Use smem_work_ptr for counter
    uint32_t save_smem_work_ptr = smem_work_ptr[0];
    if ( threadIdx.x == 0 ) {
     smem_work_ptr[0] = 0;
    }
    ...
    INDEX_T threshold_index = result_buffer_size - 1;
    const INDEX_T invalid_index_for_threshold        = utils::get_max_value<INDEX_T>();
    while(result_indices_buffer[threshold_index] == invalid_index_for_threshold) threshold_index--;
    DISTANCE_T threshold_distance = result_distances_buffer[threshold_index];
    ...
    device::compute_distance_to_child_nodes<TEAM_SIZE, DATASET_BLOCK_DIM, max_n_frags>(
      ...
      metric,
      threshold_distance,
      use_direction); //TODO (1, smem_work_ptr) is needed for dynamic code version

    // TODO: Restore smem_work_ptr
    if ( threadIdx.x == 0 ) {
     smem_work_ptr[0] = save_smem_work_ptr;
    }
