// Cosine similarity based approximation test
// Version random direction prune half
  ...
  int32_t use_direction = 0)    // TODO
{
  ...
  if (use_direction)
  {
    max_i = knn_k * search_width / 64;
  }
  else
  {
    max_i = knn_k * search_width;
  }
  if (max_i % (32 / TEAM_SIZE)) { max_i += (32 / TEAM_SIZE) - (max_i % (32 / TEAM_SIZE)); }
  for (std::uint32_t tid = threadIdx.x; tid < max_i * TEAM_SIZE; tid += blockDim.x) {
    const auto i       = tid / TEAM_SIZE;
    const bool valid_i = (i < (( knn_k * search_width) / 64));
    INDEX_T child_id   = invalid_index;
