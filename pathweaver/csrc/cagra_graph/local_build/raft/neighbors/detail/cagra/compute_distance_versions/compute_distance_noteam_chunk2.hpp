  ...
  uint32_t* counter = nullptr)
{
  ...
  {
        int32_t team_size = 2;
        int32_t team_id = (threadIdx.x % 32) / team_size;
        int32_t lane_id = threadIdx.x % 2;
        uint32_t team_mask = 0x3 << (team_id * team_size);
        ...
        while(true)
        {
              ...
              uint32_t i = __shfl_sync(team_mask, i_global, team_id * team_size);
...
struct standard_dataset_descriptor_t
                                          ...
                                          ) const
  {
    ...
    {   
          ...
          const auto dataset_ptr  = ptr + dataset_i * ld;
          const unsigned lane_id  = threadIdx.x % 2;
          int32_t team_size = 2;
          int32_t team_id = (threadIdx.x % 32) / team_size;
          uint32_t team_mask = 0x3 << (team_id * team_size);
          ...
          if (valid)
          {
              ...
              // #endif
              #pragma unroll
              for (uint32_t iter = 0; iter < dim; iter += 2)
              {   
                  ...
                  DISTANCE_T norm2_partial = dist_op<DISTANCE_T, METRIC>(d, spatial::knn::detail::utils::mapping<float>{}(dataset_ptr[lane_id + iter]));
                  // DISTANCE_T norm2_partial = (query_ptr[lane_id + iter] - static_cast<float>(dataset_ptr[lane_id + iter])) * (query_ptr[lane_id + iter] - static_cast<float>(dataset_ptr[lane_id + iter]));

                  // shfl reduce to get partial sum
                  norm2_partial += __shfl_xor_sync(team_mask, norm2_partial, 1);
