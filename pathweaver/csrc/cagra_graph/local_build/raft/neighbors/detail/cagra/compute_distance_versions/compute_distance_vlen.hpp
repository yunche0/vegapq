...
struct standard_dataset_descriptor_t
                                          ...
                                          ) const
  {
    ...
    DISTANCE_T norm2_2 = 0;
    DISTANCE_T norm2_3 = 0;
    DISTANCE_T norm2_4 = 0;
    ...
    {             
                  ...
                  const unsigned team_id  = threadIdx.x / TEAM_SIZE;
                  unsigned int team_mask = 0xff << (team_id * 8);
                            ...
                            for (uint32_t elem_offset = 0; elem_offset < dim; elem_offset += DATASET_BLOCK_DIM) 
                            {
                              ...
                              #pragma unroll
                                      for (uint32_t e = 0; e < reg_nelem; e++) 
                                      {
                                        ...
                                        if (k >= dim) break;
                              #pragma unroll
                                        for (uint32_t v = 0; v < vlen/2; v++) {
                                          const uint32_t kv = k + v;
                                          DISTANCE_T d = query_ptr[device::swizzling(kv)];
                                          norm2_1 += dist_op<DISTANCE_T, METRIC>(
                                            d, spatial::knn::detail::utils::mapping<float>{}(dl_buff[e].val.data[v]));
                                        }
                                      }
                            }
                  ...
                  // add break if norm2_1 bigger than thres
                  if(norm2_1 > thres){ return ignore_value; }
                            ...
                            for (uint32_t elem_offset = 0; elem_offset < dim; elem_offset += DATASET_BLOCK_DIM) 
                            {
                              ...
                              #pragma unroll
                                      for (uint32_t e = 0; e < reg_nelem; e++) 
                                      {
                                        ...
                                        if (k >= dim) break;
                              #pragma unroll
                                        for (uint32_t v = vlen/2; v < vlen/2 + vlen/4; v++) {
                                          const uint32_t kv = k + v;
                                          DISTANCE_T d = query_ptr[device::swizzling(kv)];
                                          norm2_2 += dist_op<DISTANCE_T, METRIC>(
                                            d, spatial::knn::detail::utils::mapping<float>{}(dl_buff[e].val.data[v]));
                                        }
                                      }         
                            }
                  ...
                  for (uint32_t offset = TEAM_SIZE / 2; offset > 0; offset >>= 1) 
                  {
                    norm2_2 += __shfl_xor_sync(team_mask, norm2_2, offset);
                  }

                  // add break if norm2_1 + norm2_2 bigger than thres
                  if(norm2_1 + norm2_2 > thres) { return ignore_value; }
                  //////////////////////////////////////////////////////////////////////////
                            ...
                            for (uint32_t elem_offset = 0; elem_offset < dim; elem_offset += DATASET_BLOCK_DIM) 
                            {
                              ...
                              #pragma unroll
                                      for (uint32_t e = 0; e < reg_nelem; e++) 
                                      {
                                        ...
                                        if (k >= dim) break;
                              #pragma unroll
                                        for (uint32_t v = vlen/2 + vlen/4; v < vlen/2 + vlen/4 + vlen/8; v++) {
                                          const uint32_t kv = k + v;
                                          DISTANCE_T d = query_ptr[device::swizzling(kv)];
                                          norm2_3 += dist_op<DISTANCE_T, METRIC>(
                                            d, spatial::knn::detail::utils::mapping<float>{}(dl_buff[e].val.data[v]));
                                        }
                                      }         
                            }
                  ...
                  for (uint32_t offset = TEAM_SIZE / 2; offset > 0; offset >>= 1) 
                  {
                    norm2_3 += __shfl_xor_sync(team_mask, norm2_3, offset);
                  }

                  // add break if norm2_1 + norm2_2 bigger than thres
                  if(norm2_1 + norm2_2 + norm2_3 > thres) { return ignore_value; }
                            ...
                            for (uint32_t elem_offset = 0; elem_offset < dim; elem_offset += DATASET_BLOCK_DIM) 
                            {
                              ...
                              #pragma unroll
                                      for (uint32_t e = 0; e < reg_nelem; e++) 
                                      {
                                        ...
                                        if (k >= dim) break;
                              #pragma unroll
                                        for (uint32_t v = vlen/2 + vlen/4 + vlen/8; v < vlen; v++) {
                                          const uint32_t kv = k + v;
                                          DISTANCE_T d = query_ptr[device::swizzling(kv)];
                                          norm2_4 += dist_op<DISTANCE_T, METRIC>(
                                            d, spatial::knn::detail::utils::mapping<float>{}(dl_buff[e].val.data[v]));
                                        }
                                      }         
                            }
                  ...
                  for (uint32_t offset = TEAM_SIZE / 2; offset > 0; offset >>= 1) 
                  {
                    norm2_4 += __shfl_xor_sync(team_mask, norm2_4, offset);
                  }


                  norm2 = norm2_1 + norm2_2 + norm2_3 + norm2_4;
