// Dynamic scheduling of work load
// relem version
// chunk size 32
// Is this 35% in sift?
  ...
  uint32_t* counter = nullptr)
{
  ...
  if(is_dynamic)
  {
        ...
        {
          case 1:
            team_mask = 0x1 << ( team_id * TEAM_SIZE );
            break;
          case 2:
            team_mask = 0x3 << ( team_id * TEAM_SIZE );
            break;
          case 4:
            team_mask = 0xf << ( team_id * TEAM_SIZE );
            break;
          case 8:
            team_mask = 0xff << ( team_id * TEAM_SIZE );
            break;
          case 16:
            team_mask = 0xffff << ( team_id * TEAM_SIZE );
            break;
          case 32:
            team_mask = 0xffffffff;
            break;
          default:
        ...
        {
              uint32_t i_atomic = 0;
              // Group team into size of warp and warp sync before start
              if(lane_id ==  0)
              { 
                  i_atomic = atomicAdd(counter, 1);
              }
              uint32_t i = __shfl_sync(team_mask, i_atomic, team_id * TEAM_SIZE);
...
struct standard_dataset_descriptor_t
                                          ...
                                          ) const
  {
    ...
    {             
          ...
          // TODO
          // constexpr unsigned vlen = device::get_vlen<LOAD_T, DATA_T>();
          constexpr unsigned vlen = 1;
          // #include <raft/util/cuda_dev_essentials.cuh
          constexpr unsigned reg_nelem = raft::ceildiv<unsigned>(DATASET_BLOCK_DIM, TEAM_SIZE * vlen);
          raft::TxN_t<DATA_T, vlen> dl_buff[reg_nelem];

          const unsigned team_id  = (threadIdx.x % 32) / TEAM_SIZE;
