// DISTANCE NO TEAM version
// Probably functionaliy issue
  ...
  uint32_t* counter = nullptr)
{
  ...
  if(is_dynamic)
  {
        ...
        while(true)
        {
              ...
              if(lane_id ==  0)
              {
                i_global = atomicAdd(counter, 1);
              }
              uint32_t i = __shfl_sync(full_mask, i_global, 0);
  ...
  {   
        printf("ERROR STATIC METHOD NOT DONE\n");
  }
...
struct standard_dataset_descriptor_t
                                          ...
                                          ) const
  {
    ...
    {   
          ...
          DISTANCE_T ignore_value = utils::get_max_value<DISTANCE_T>();
          ...
          {
              #pragma unroll
              for (uint32_t iter = 0; iter < dim; iter += 32)
              {   

                  // Load 32 elements
                  // Compute single distance per threads
                  // TODO is there bank conflict?
                  raft::TxN_t<DATA_T, 1> dl_buff;
                  dl_buff.load(dataset_ptr, lane_id + iter);
                
                  DISTANCE_T d = query_ptr[lane_id + iter];
                  DISTANCE_T norm2_partial = dist_op<DISTANCE_T, METRIC>(d, spatial::knn::detail::utils::mapping<float>{}(dl_buff.val.data[0]));
 
                  if (threadIdx.x == 0 && blockIdx.x == 0) {
                      printf("Iter: %u, lane_id: %u\n", iter, lane_id);
                      printf("dl_buff.val.data[0]: %f\n", static_cast<float>(dl_buff.val.data[0]));
                      printf("d: %f, norm2_partial (initial): %f\n", static_cast<float>(d), static_cast<float>(norm2_partial));
                  }


                  // shfl reduce to get partial sum
                  for (uint32_t offset = 16 ; offset > 0; offset >>= 1) 
                  {
                    norm2_partial += __shfl_xor_sync(full_mask, norm2_partial, offset);
                  }

                  // Check if the distance is bigger than threshold
                  // If set valid to 0, max value for distance will be set on child distance kernel
                  // break
                  norm2 = norm2 + norm2_partial;
                  if (norm2 > thres) {
                      return ignore_value;
                  }
          ...
          // unsigned int team_mask = 0xffffffff;

          // if (threadIdx.x == 0 && blockIdx.x == 0) { // 하나의 스레드에서만 출력
          //     printf("Size of DATA_T: %lu bytes\n", static_cast<unsigned long>(sizeof(DATA_T)));
          //     printf("Size of QUERY_T: %lu bytes\n", static_cast<unsigned long>(sizeof(QUERY_T)));
          //     printf("Size of DISTANCE_T: %lu bytes\n", static_cast<unsigned long>(sizeof(DISTANCE_T)));
          //     printf("Size of Dataset element: %lu bytes\n", static_cast<unsigned long>(sizeof(dataset_ptr[0])));
          // }
          ...
          //                                     {
          //                                         const uint32_t kv = k + v;
          //                                         DISTANCE_T d = query_ptr[kv] - spatial::knn::detail::utils::mapping<float>{}(dl_buff[e].val.data[v]);
          //                                         norm2_1 += d * d;
          ...
          //                 // TODO Check sync and load imbalance
          //                 for (uint32_t offset = team_size / 2; offset > 0; offset >>= 1) 
          //                 {
          //                   norm2_1 += __shfl_xor_sync(team_mask, norm2_1, offset, team_size);
          //                 }
